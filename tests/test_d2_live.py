"""Tests for the local D2 live editor sidecar."""
...
from anno.d2_live import LIVE_HTML, _write_state, preview_url_if_running, start_live_server


def test_live_html_is_an_editor_page():
    assert "d2" in LIVE_HTML.lower()
    assert "@terrastruct/d2" in LIVE_HTML
    ...
    assert "function applyDisk" in LIVE_HTML
    assert "loaded from disk" in LIVE_HTML
    assert "compileOne" in LIVE_HTML
    assert "rendered (softened notes)" in LIVE_HTML
    assert 'id="gutter"' in LIVE_HTML
    assert "col-resize" in LIVE_HTML
    assert "anno-d2-split" in LIVE_HTML
    assert 'id="saveNow"' in LIVE_HTML
    assert 'id="checkNow"' in LIVE_HTML
    assert "compile failed:" in LIVE_HTML
    assert "valid D2" in LIVE_HTML
    assert 'id="reconnect"' in LIVE_HTML
    assert 'id="zoomIn"' in LIVE_HTML
    assert 'id="zoomOut"' in LIVE_HTML
    assert 'id="zoomFit"' in LIVE_HTML
    assert 'id="stage"' in LIVE_HTML
    assert "wheel" in LIVE_HTML
    assert "pinWorld" in LIVE_HTML
    assert "ctrlKey" in LIVE_HTML
    assert "viewBox" in LIVE_HTML
    assert "clientToWorld" in LIVE_HTML
    assert "lastPtr" in LIVE_HTML
    assert "cam.x = mapped.x - mapped.nx * nextW" in LIVE_HTML
    assert 'translate(" + tx + "px," + ty + "px) scale(' not in LIVE_HTML
    assert "max-width: none" in LIVE_HTML
    assert "snapEditor" in LIVE_HTML
    assert "objectIdFromNode" in LIVE_HTML
    assert "findObjectSpan" in LIVE_HTML
    assert "setSelectionRange" in LIVE_HTML
    assert "cursor: pointer" in LIVE_HTML
    assert 'id="hl"' in LIVE_HTML
    assert "highlightD2" in LIVE_HTML
    assert "paintHighlight" in LIVE_HTML
    assert "caret-color" in LIVE_HTML
    assert "tok-c" in LIVE_HTML
    assert "tok-e" in LIVE_HTML
...
def test_live_server_rejects_unknown_post(tmp_path: Path):
    path = tmp_path / "pipe.d2"
    path.write_text("x\n")
...
def test_preview_url_if_running_reads_state(tmp_path: Path, monkeypatch):
    monkeypatch.setenv("ANNO_D2_LIVE_DIR", str(tmp_path / "state"))
    path = tmp_path / "x.d2"
    path.write_text("x\n")
    httpd, port, _done = start_live_server(path)
    try:
